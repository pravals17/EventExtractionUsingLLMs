...
def build_five_shot_messages(five_shot_df, document_text):
    ...
    for _, row in five_shot_df.iterrows():
        ...
        messages.append({
            "role": "user",
            "content": [{"type": "text", "text": example_prompt}],
        })

        messages.append({
            "role": "assistant",
            "content": [{"type": "text", "text": example_answer}],
        })
...
def main():
    ...
    model = Gemma3ForConditionalGeneration.from_pretrained(
        model_name, dtype="auto", device_map="auto", cache_dir="/Gemma312B/cache"
    )
	
    processor = AutoProcessor.from_pretrained(model_name, cache_dir="/Gemma312B/cache")
